...
class SocketAddress6 {
	...
	var stringValue: String? {
		return withUnsafePointer(to: &sin6) { saToString(UnsafePointer<sockaddr>($0)) }
	}
...
class SocketAddress {
	...
	var stringValue: String? {
		return withUnsafePointer(to: &sin) { saToString(UnsafePointer<sockaddr>($0)) }
	}
...
public func getValueFromPlist(_ plist: [AnyHashable: Any], keyArray: [SettingsKey]) -> AnyObject? {
	var subPlist = plist
	for (index, key) in keyArray.enumerated() {
		if index == keyArray.count - 1 {
			return subPlist[key.rawValue] as AnyObject?
		}
		else if let subSubPlist = subPlist[key.rawValue] as? [AnyHashable: Any] {
